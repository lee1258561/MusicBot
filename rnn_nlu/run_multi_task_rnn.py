...
tf.app.flags.DEFINE_string("data_dir", "rnn_nlu/data/", "Data directory")
tf.app.flags.DEFINE_string("train_dir", "rnn_nlu/model_tmp", "Training directory.")
...
class NLU_test(object): 
    def __init__(self):
        ...
        tag_vocab_path = ''
        label_vocab_path = ''
        in_seq_train, out_seq_train, label_train, in_seq_dev, out_seq_dev, label_dev, in_seq_test, out_seq_test, label_test, vocab_path, tag_vocab_path, label_vocab_path = data_utils.prepare_multi_task_data(FLAGS.data_dir, FLAGS.in_vocab_size, FLAGS.out_vocab_size)
        ...
        self.new_vocab = {}
        for w in self.vocab :
            self.new_vocab[w.decode('utf-8')] = self.vocab[w]
    ...
    def feed_sentence(self, sentence):
        data_set = [[]]
        token_ids = data_utils.prepare_one_data(sentence, self.new_vocab)
        print(token_ids)

        slot_ids = [0 for i in range(len(token_ids))]
        data_set[0].append([token_ids, slot_ids, [0]])
        encoder_inputs, tags, tag_weights, sequence_length, labels = self.model_test.get_one(data_set, 0, 0)
        if task['joint'] == 1:
